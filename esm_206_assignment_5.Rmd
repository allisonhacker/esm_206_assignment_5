...
# Data does not seem normally distributed... however, the large sample sizes make it so the means will be normally distributed (Central Limit Theorem),so let's check for equal variance to ensure we can use a parametric test
...
leveneTest(weight ~ unittype, data = channel_weight)
...
# p = 0.0151, so this is statistically significant. But are they all the same? Or just 2? > comparing to type 3 anova

library(car)

channel_lm <- lm(weight ~ unittype, data = channel_weight)

channel_aov_t3 <- Anova(channel_lm)

channel_aov_t3

# F value statistic is ~ the same
...
The mean weight for salamanders in side channels (5.68 ± 0.65g, mean ± standard error, n = 164) was lower than for those in cascades (7.52 ± 0.43g, n = 447) and pools (9.30 ± 1.56g, n = 76). Mean salamander weight differed significantly between salamanders in side channels and in pools (One-way ANOVA with post-hoc Tukey's HSD, F(2) = 4.22, p = 0.016), but did not differ significantly between those in pools and cascades (p = 0.28) or between those in side channels and cascades (p = 0.084). 
