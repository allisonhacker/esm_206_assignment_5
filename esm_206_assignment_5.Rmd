...
We analyzed salamander weights in old growth and clear cut sections of Mack Creek for 2017. Mean salamander weight based in clear cut (`r round(section_weight_summary$mean[1], 2)` ± `r round(section_weight_summary$se[1], 2)`, mean ± standard error, n = `r round(section_weight_summary$count[1], 2)`) and old growth (`r round(section_weight_summary$mean[2], 2)` ± `r round(section_weight_summary$se[2], 2)`, n = `r round(section_weight_summary$count[2], 2)`) forest in 2017 did not differ significantly (two-sample, two-sided t-test, t(`r round(weight_ttest$parameter, 0)`) = `r round(weight_ttest$statistic, 1)`, *p* = `r round(weight_ttest$p.value, 3)`, Cohen's d = `r round(section_weight_es$estimate, 2)`).
